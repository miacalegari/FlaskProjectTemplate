...
original_name="flaskprojecttemplate"
original_urlname="FlaskProjectTemplate"
original_description="Awesome flaskprojecttemplate created by miacalegari" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flaskprojecttemplate "${name}"
...
then
    ...
    echo "then run 'flaskprojecttemplate' or 'python -m flaskprojecttemplate'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flaskprojecttemplate' or 'python -m flaskprojecttemplate' or 'poetry run flaskprojecttemplate'"
fi
